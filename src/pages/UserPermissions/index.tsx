...
const THEME = {
  ...
  primary: '#022d41', // Deep Navy
  primaryLight: '#1aa6b7', // Blue
  accent: '#ffa64a', // Orange/Ochre
  gold: '#c3924c',
  brightGold: '#af7a2b',
  success: '#398797', // Green
  danger: '#fe424d', // Salmon
  skyBlue: '#5f7ab7',
  dustyBlue: '#a3c2d2',
  indigo: '#5f7ab7',
  softPurple: '#9293c3',
...
const PERMISSION_LEVELS = [
  { level: 0, label: 'No Access', icon: 'ban', color: THEME.dustyBlue, bg: '#daecf3' },
  { level: 1, label: 'Viewer', icon: 'eye', color: THEME.skyBlue, bg: '#daecf3' },
  { level: 2, label: 'Editor', icon: 'edit', color: THEME.accent, bg: '#ffa64a40' },
  { level: 3, label: 'Verifier', icon: 'check-square', color: THEME.indigo, bg: '#a3c2d2' },
  { level: 4, label: 'Approver', icon: 'award', color: THEME.success, bg: '#39879740' },
];
...
const PermissionBadges = ({ user, moduleId }: any) => {
    ...
    if (!perms || perms.length === 0) {
        return <span className="text-[#a3c2d2] font-black text-[14px] opacity-50">-</span>;
    }
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#af7a2b] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-4 px-6 border-b-2 border-[#af7a2b] bg-[#022d41] text-white shrink-0">
          ...
          <button onClick={onClose} className="p-2 text-white/50 hover:text-[#fe424d] hover:bg-white/10 rounded-xl transition-colors"><Icons.X size={24}/></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-[#214573] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            ...
            <div className="space-y-3 font-medium bg-[#f8f9fa] p-4 rounded-xl border border-[#daecf3] shadow-sm">
                <div className="flex items-center gap-3"><button className="bg-[#022d41] text-white px-3 py-1 rounded-full text-[10px] font-black uppercase shadow-sm flex items-center gap-1"><Icons.UserPlus size={12}/> New User</button> <span className="text-[11px] text-[#214573]">เพิ่มผู้ใช้งานใหม่เข้าสู่ระบบ</span></div>
                <div className="flex items-center gap-3"><button className="w-6 h-6 flex items-center justify-center bg-white text-[#af7a2b] border border-[#daecf3] rounded-lg shadow-sm"><Icons.Edit size={12}/></button> <span className="text-[11px] text-[#214573]">แก้ไขข้อมูลและสิทธิ์ของผู้ใช้งานในแถวนั้นๆ</span></div>
                <div className="flex items-center gap-3"><button className="w-6 h-6 flex items-center justify-center bg-white text-[#fe424d] border border-[#daecf3] rounded-lg shadow-sm"><Icons.Trash2 size={12}/></button> <span className="text-[11px] text-[#214573]">ลบผู้ใช้งานออกจากระบบ</span></div>
                <div className="flex items-center gap-3"><div className="relative"><Icons.Search size={12} className="absolute left-2 top-1.5 text-[#a3c2d2]"/><input placeholder="Search..." className="pl-6 py-1 text-[10px] bg-white border border-[#daecf3] rounded-lg w-20 pointer-events-none"/></div> <span className="text-[11px] text-[#214573]">ค้นหาข้อมูลผู้ใช้ในตาราง</span></div>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#022d41] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#e7dedd] pb-2 font-mono">
              <Icons.ShieldAlert size={18} className="text-[#af7a2b]"/> 2. Confidential Restricted
            ...
            <ul className="list-none pl-0 space-y-3">
                <li className="flex items-start gap-2 bg-[#f8f9fa] p-3 rounded-xl border border-[#daecf3]">
                  <Icons.Eye size={16} className="shrink-0 text-[#1aa6b7] mt-0.5"/> 
                  <div><strong className="text-[#1aa6b7]">Public Node:</strong> ทุกคนในระบบได้รับสิทธิ์ในการเข้าถึงและมองเห็นเบื้องต้น (Viewer) ยกเว้นโดนจำกัดสิทธิ์รายบุคคล</div>
                </li>
                <li className="flex items-start gap-2 bg-[#fe424d]/10 p-3 rounded-xl border border-[#fe424d]/30">
                  <Icons.Lock size={16} className="shrink-0 text-[#fe424d] mt-0.5"/> 
                  <div><strong className="text-[#fe424d]">Restricted Area:</strong> ปิดกั้นการมองเห็นโดยสมบูรณ์ เมนูจะถูกซ่อน ต้องกำหนดสิทธิ์รายบุคคลแบบเจาะจงเท่านั้น</div>
                </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            ...
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#1aa6b7]">Viewer (สิทธิ์การดู):</strong> สามารถเข้าถึงหน้านั้นๆ ได้ แต่อ่านข้อมูลได้อย่างเดียว</li>
                <li><strong className="text-[#ffa64a]">Editor (สิทธิ์แก้ไข):</strong> สามารถสร้าง หรือแก้ไขข้อมูลและฟอร์มต่างๆ ภายในโมดูลได้</li>
                <li><strong className="text-[#022d41]">Verifier (สิทธิ์ตรวจสอบ):</strong> มีอำนาจตรวจสอบความถูกต้อง (Verify) ก่อนส่งให้อนุมัติ</li>
                <li><strong className="text-[#398797]">Approver (สิทธิ์อนุมัติ):</strong> อำนาจขั้นสูงสุดในการอนุมัติเอกสารและคำสั่ง (Approve)</li>
            </ul>
          </section>
...
function EditUserModal({ isOpen, onClose, user, onSave }: any) {
    ...
    return (
        <DraggableModal
            ...
            width="max-w-[950px]"
            customHeader={
                <div className="bg-[#022d41] px-4 py-3 flex justify-between items-center shrink-0 border-b-2 border-[#af7a2b]">
                    ...
                    <button onClick={onClose} className="text-white/70 hover:text-[#fe424d] transition-all bg-white/10 hover:bg-white/20 p-1.5 rounded-full"><Icons.X size={16} /></button>
                </div>
            }
        >
                {/* Modal Body */}
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#f8f9fa]">
                    {/* Sidebar Steps */}
                    <div className="w-full md:w-56 bg-white border-b md:border-b-0 md:border-r border-[#daecf3] flex flex-row md:flex-col shrink-0">
                        <div className="hidden md:block px-4 py-4 text-[10px] font-black text-[#a3c2d2] uppercase tracking-widest border-b border-[#daecf3] bg-[#f8f9fa]">Configuration Nodes</div>
                        {[0, 1, 2].map(step => (
                            <button key={step} onClick={()=>setModalStep(step)} className={`flex-1 md:flex-none flex items-center justify-center md:justify-start gap-3 px-4 py-3 text-left transition-all md:border-l-4 ${modalStep===step ? 'border-b-4 md:border-b-0 border-[#af7a2b] bg-[#f8f9fa] text-[#022d41]' : 'border-transparent text-[#a3c2d2] hover:bg-[#f8f9fa]/50'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 bg-white">
                        {modalStep === 0 ? (
                            <div className="space-y-4 max-w-xl">
                                <div>
                                    <label className="block text-[11px] font-black text-[#a3c2d2] uppercase tracking-widest mb-1.5">Full Name</label>
                                    <input type="text" value={tempUser.name || ''} onChange={e => setTempUser({...tempUser, name: e.target.value})} className="w-full bg-[#f8f9fa] border border-[#daecf3] rounded-lg px-4 py-2 text-[12px] font-bold text-[#022d41] outline-none focus:border-[#af7a2b]" />
                                </div>
                                <div>
                                    <label className="block text-[11px] font-black text-[#a3c2d2] uppercase tracking-widest mb-1.5">Position / Role</label>
                                    <input type="text" value={tempUser.position || ''} onChange={e => setTempUser({...tempUser, position: e.target.value})} className="w-full bg-[#f8f9fa] border border-[#daecf3] rounded-lg px-4 py-2 text-[12px] font-bold text-[#022d41] outline-none focus:border-[#af7a2b]" />
                                </div>
                                <div>
                                    <label className="block text-[11px] font-black text-[#a3c2d2] uppercase tracking-widest mb-1.5">Email Address</label>
                                ...
                                <div className="flex items-center gap-3 pt-2">
                                    ...
                                    <label htmlFor="isDevCheck" className="text-[11px] font-black text-[#022d41] uppercase tracking-widest cursor-pointer">Super Admin (Developer) Privileges</label>
                                </div>
                            </div>
                        ) : (
                        <div className="space-y-3">
                            {SYSTEM_MODULES.map(mod => {
                                ...
                                return (
                                    <div key={mod.id} className={`px-4 py-3 rounded-xl border transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-3 ${userHasMod || isDev ? 'bg-[#f8f9fa] border-[#af7a2b]/50 shadow-sm' : 'bg-white border-[#daecf3]'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border shrink-0 ${userHasMod || isDev ? 'bg-[#022d41] text-white border-[#022d41]' : 'bg-[#f8f9fa] text-[#a3c2d2] border-[#daecf3]'}`}>
                                                <LucideIcon name={mod.icon} size={16}/>
                                            </div>
                                            <span className="font-black text-[#022d41] uppercase text-[11px] tracking-widest leading-tight">{mod.label}</span>
                                        </div>
                                        <div className="flex flex-wrap gap-1.5 p-1.5 bg-white rounded-lg border border-[#daecf3] w-full md:w-auto">
                                            {PERMISSION_LEVELS.filter(p => modalStep===1 ? p.level <= 1 : p.level===0 || p.level>=2).map(p => {
                                                ...
                                                return (
                                                    <button key={p.level} onClick={()=>handleTogglePerm(mod.id, p.level)} disabled={isDev} className={`flex-1 md:flex-none h-8 px-2.5 rounded-md border flex items-center justify-center gap-1.5 transition-all ${isSelected ? 'bg-[#022d41] text-white border-[#022d41] shadow-sm' : 'bg-white border-transparent text-[#a3c2d2] hover:bg-[#f8f9fa]'}`}>
                ...
                {/* Modal Footer */}
                <div className="px-6 py-3 bg-[#f8f9fa] border-t border-[#daecf3] flex justify-end gap-3 shrink-0">
                    <button onClick={onClose} className="px-5 py-2 bg-white border border-[#daecf3] text-[#214573] rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-[#e7dedd]/30 transition-all">Cancel</button>
                    <button onClick={()=>{onSave(tempUser, tempPerms); onClose();}} className="bg-[#022d41] text-white px-5 py-2 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-md hover:bg-[#214573] hover:text-white transition-all flex items-center gap-2"><Icons.Save size={14}/> Save User</button>
                </div>
        </DraggableModal>
    );
}
    ...
    <div className="flex flex-1 w-full font-sans flex-col pb-0 animate-fadeIn bg-transparent">
      ...
      <div className="px-8 pt-3 pb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 z-20 shrink-0">
          <div className="flex items-center gap-5">
              ...
              <div>
                  <h3 className="font-black text-[#022d41] uppercase tracking-tighter leading-none" style={{ fontSize: '24px' }}>
                      USER <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8B0000] to-[#FFD700]">PERMISSIONS</span> NODE
                  </h3>
          ...
          <div className="flex items-center gap-4">
              <div className="bg-white/50 p-1.5 rounded-xl border border-white/60 shadow-inner flex flex-wrap items-center gap-1">
                  <button onClick={() => setActiveTab('registry')} className={`px-6 py-2.5 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'registry' ? 'bg-[#022d41] text-white shadow-md' : 'text-[#a3c2d2] hover:text-[#f91a47]'}`}>
                    <Icons.Database size={16} /> Global Registry
                  </button>
                  <button onClick={() => setActiveTab('staff')} className={`px-6 py-2.5 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'staff' ? 'bg-[#022d41] text-white shadow-md' : 'text-[#a3c2d2] hover:text-[#f91a47]'}`}>
        ...
        <div className="max-w-[1500px] w-full mx-auto">
            ...
            {activeTab === 'registry' ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-4 bg-white/90 p-8 rounded-3xl shadow-lg border border-[#daecf3] animate-fadeIn h-fit">
                        <h3 className="text-[14px] font-black text-[#022d41] uppercase tracking-widest flex items-center gap-3 border-b-2 border-[#af7a2b] pb-4 mb-6"><Icons.ShieldAlert size={20} className="text-[#af7a2b]" /> ACCESS POLICIES</h3>
                        <div className="space-y-5">
                            <div className="p-5 bg-[#f8f9fa] border border-[#daecf3] rounded-2xl shadow-sm hover:border-[#1aa6b7] transition-colors">
                                <div className="flex items-center gap-2 text-[#1aa6b7] font-black text-[12px] uppercase tracking-widest mb-2"><Icons.Eye size={18}/> Public Node</div>
                                <p className="text-[12px] text-[#214573] font-bold leading-relaxed">โมดูลมาตรฐาน: พนักงานจะได้รับสิทธิ์อ่าน (Viewer) เบื้องต้นโดยอัตโนมัติ</p>
                            </div>
                            <div className="p-5 bg-[#fe424d]/10 border border-[#fe424d]/30 rounded-2xl shadow-sm hover:border-[#fe424d] transition-colors">
                                <div className="flex items-center gap-2 text-[#fe424d] font-black text-[12px] uppercase tracking-widest mb-2"><Icons.Lock size={18}/> Restricted Area</div>
                                <p className="text-[12px] text-[#214573] font-bold leading-relaxed">พื้นที่จำกัด: เมนูจะถูกซ่อนจากผู้ใช้ทั่วไป ต้องได้รับสิทธิ์แบบเจาะจงรายบุคคลเท่านั้น</p>
                            </div>
                        </div>
                    </div>

                    {/* GLOBAL MODULE REGISTRY (SYNCED) */}
                    <div className="lg:col-span-8 bg-white rounded-3xl shadow-lg border border-[#daecf3] overflow-hidden">
                        <div className="p-6 bg-[#f8f9fa] border-b border-[#daecf3]">
                            <h4 className="text-[14px] font-black uppercase text-[#022d41] tracking-widest flex items-center gap-3"><Icons.ListTree size={20} className="text-[#af7a2b]"/> GLOBAL MODULE REGISTRY</h4>
                        </div>
                        <div className="p-6 space-y-3 custom-scrollbar">
                            {SYSTEM_MODULES.map(mod => (
                                <div key={mod.id} className="space-y-2">
                                    <div className={`flex items-center justify-between px-4 py-2.5 rounded-2xl border transition-all ${confidentialityMap[mod.id] ? 'bg-[#fe424d]/5 border-[#fe424d]/30 shadow-sm' : 'bg-white border-[#daecf3] hover:border-[#1aa6b7]'}`}>
                                        <div className="flex items-center gap-4">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-sm ${confidentialityMap[mod.id] ? 'bg-[#fe424d]/20 text-[#fe424d] border-[#fe424d]/30' : 'bg-[#f8f9fa] text-[#022d41] border-[#daecf3]'}`}>
                                                <LucideIcon name={mod.icon} size={22}/>
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-black text-[#022d41] text-[13px] uppercase tracking-widest">{mod.label}</span>
                                                    {mod.subItems && (
                                                ...
                                                <span className={`text-[11px] font-black uppercase tracking-widest ${confidentialityMap[mod.id] ? 'text-[#fe424d]' : 'text-[#a3c2d2]'}`}>Module {confidentialityMap[mod.id] ? 'Restricted' : 'Public'}</span>
                                            </div>
                                        </div>
                                        <button onClick={()=>toggleConfidentiality(mod.id)} className={`p-2.5 rounded-xl transition-all shadow-sm active:scale-90 ${confidentialityMap[mod.id] ? 'bg-[#fe424d] text-white' : 'bg-white text-[#a3c2d2] border border-[#daecf3] hover:bg-[#f8f9fa]'}`}>
                                    ...
                                    {mod.subItems && expandedModules[mod.id] && (
                                        <div className="ml-16 space-y-2 animate-fadeIn pr-4 pb-4">
                                            {mod.subItems.map((sub: any) => (
                                                <div key={sub.id} className={`flex items-center justify-between px-4 py-2.5 rounded-xl border bg-white transition-all ${confidentialityMap[sub.id] ? 'border-[#fe424d]/40 bg-[#fe424d]/5 shadow-inner' : 'border-[#e7dedd] hover:border-[#1aa6b7]'}`}>
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-2 h-2 rounded-full ${confidentialityMap[sub.id] ? 'bg-[#fe424d] animate-pulse' : 'bg-[#af7a2b]'}`}></div>
                                                        <span className="text-[12px] font-black text-[#022d41] uppercase tracking-widest">{sub.label}</span>
                                                    </div>
                                                    <button onClick={()=>toggleConfidentiality(sub.id)} className={`p-2 rounded-lg transition-all ${confidentialityMap[sub.id] ? 'bg-[#fe424d]/10 text-[#fe424d]' : 'text-[#a3c2d2] hover:bg-[#f8f9fa]'}`}>
                ...
                <div className="bg-white rounded-3xl shadow-lg border border-[#daecf3] overflow-hidden flex flex-col animate-fadeIn">
                    <div className="px-8 py-4 border-b border-[#daecf3] bg-[#f8f9fa] flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="flex bg-[#f8f9fa] border border-[#daecf3] p-1 rounded-full shadow-sm inline-flex">
                                <button onClick={()=>setViewMode('list')} className={`px-5 py-2 text-[11px] font-black uppercase tracking-widest rounded-full transition-all flex items-center gap-2 ${viewMode==='list'?'bg-[#022d41] text-[#e7dedd] shadow-md':'text-[#a3c2d2] hover:text-[#f91a47]'}`}>
                                    <Icons.List size={14}/> List View
                                </button>
                                <button onClick={()=>setViewMode('matrix')} className={`px-5 py-2 text-[11px] font-black uppercase tracking-widest rounded-full transition-all flex items-center gap-2 ${viewMode==='matrix'?'bg-[#022d41] text-[#e7dedd] shadow-md':'text-[#a3c2d2] hover:text-[#f91a47]'}`}>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <div className="relative flex-1 md:w-80">
                                <Icons.Search size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#a3c2d2]" />
                                <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search personnel..." className="w-full pl-12 pr-6 py-2.5 text-[12px] border border-[#daecf3] rounded-full font-bold outline-none focus:border-[#af7a2b] bg-white shadow-sm text-[#022d41]" />
                            </div>
                            <button onClick={()=>setEditModal({isOpen: true, user: { id: Date.now(), name: 'NEW USER', position: 'POSITION', email: '', avatar: 'https://cdn-icons-png.flaticon.com/512/149/149071.png', isDev: false, permissions: {} }})} className="bg-[#022d41] text-white px-6 py-2.5 rounded-full font-black text-[12px] uppercase tracking-widest shadow-md hover:bg-[#214573] hover:text-white transition-all flex items-center gap-2 shrink-0 border border-[#022d41]">
                    ...
                    <div className="overflow-auto custom-scrollbar">
                        {viewMode === 'list' ? (
                            <table className="w-full text-left font-sans border-collapse">
                                <thead className="bg-[#022d41] text-white">
                                ...
                                <tbody className="bg-white divide-y divide-[#daecf3]">
                                    {currentData.map(user => (
                                        <tr key={user.id} className="hover:bg-[#f8f9fa] transition-colors group">
                                            <td className="py-3 px-6 font-black text-[#022d41] uppercase tracking-tight text-[12px]">
                                                <div className="flex items-center gap-4">
                                                    <img src={user.avatar} className="w-10 h-10 rounded-xl object-cover border border-[#daecf3] shadow-sm" />
                                                    <span>{user.name}</span>
                                                </div>
                                            </td>
                                            <td className="py-3 px-6 font-bold text-[#1aa6b7] uppercase text-[12px]">{user.position}</td>
                                            <td className="py-3 px-6 font-mono text-[#a3c2d2] text-[12px]">{user.email}</td>
                                            <td className="py-3 px-6 text-center">
                                                <span className={`px-3 py-1 rounded-full text-[11px] font-black uppercase border tracking-widest ${user.isDev ? 'bg-[#af7a2b]/10 text-[#ffa64a] border-[#af7a2b]/30' : 'bg-[#022d41]/5 text-[#022d41] border-[#daecf3]'}`}>
                                                    {user.isDev ? 'Developer' : 'General'}
                                                </span>
                                            </td>
                                            <td className="py-3 px-6 text-center">
                                                <div className="flex justify-center items-center gap-[0.5px]">
                                                    <button onClick={()=>setEditModal({isOpen: true, user: user})} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#1aa6b7] bg-[#1aa6b7]/10 hover:bg-[#1aa6b7]/20 transition-all active:scale-90">
                                                        <Icons.UserCog size={16} />
                                                    </button>
                                                    <button className="w-8 h-8 flex items-center justify-center rounded-lg text-[#fe424d] bg-[#fe424d]/10 hover:bg-[#fe424d]/20 transition-all active:scale-90">
                                    ...
                                    {SYSTEM_MODULES.map(mod => (
                                        <React.Fragment key={mod.id}>
                                            <tr className="hover:bg-[#f8f9fa] transition-colors group">
                                                <td className="py-3 px-6 border-r border-[#daecf3]">
                                                    <div className="flex items-center gap-3">
                                                        {mod.subItems ? (
                                                            <button onClick={() => toggleExpand(mod.id)} className="text-[#1aa6b7] p-0.5 rounded outline-none">
                                                        ...
                                                        <span className="font-black text-[#022d41] uppercase text-[12px] tracking-widest">{mod.label}</span>
                                                    </div>
                                                </td>
                                                {currentData.map(user => (
                                                    <td key={user.id} className="py-3 px-4 text-center border-r border-[#daecf3] last:border-0">
                                                        <PermissionBadges user={user} moduleId={mod.id} />
                                                    </td>
                                                ))}
                                            </tr>
                                            {mod.subItems && expandedModules[mod.id] && mod.subItems.map((sub: any) => (
                                                <tr key={sub.id} className="bg-[#f8f9fa]/50 hover:bg-[#f8f9fa] transition-colors border-b border-[#daecf3]/50 last:border-b-0">
                                                    <td className="py-3 px-6 pl-14 border-r border-[#daecf3]">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-1 h-1 rounded-full bg-[#a3c2d2]"></div>
                                                            <span className="font-bold text-[#214573] uppercase text-[11px] tracking-wider">{sub.label}</span>
                                                        </div>
                                                    </td>
                                                    {currentData.map(user => (
                                                        <td key={user.id} className="py-3 px-4 text-center border-r border-[#daecf3] last:border-0">
                    ...
                    <div className="px-8 py-3 bg-[#f8f9fa] border-t border-[#daecf3] flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                        <div className="flex items-center gap-6 text-[11px] font-black text-[#a3c2d2] uppercase tracking-widest">
                            ...
                            <p className="bg-white px-4 py-2 rounded-xl border border-[#daecf3] shadow-sm">Total Records: {filteredUsers.length}</p>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className={`w-10 h-10 border border-[#daecf3] bg-white rounded-xl flex items-center justify-center transition-all ${currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#022d41] hover:text-white shadow-md active:scale-90'}`}>
