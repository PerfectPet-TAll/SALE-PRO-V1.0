...
const KpiCard = ({ icon: IconComp, value, label, colorAccent, colorValue, desc }: any) => (
    <div className="bg-white px-6 py-6 rounded-3xl border border-[#daecf3]/60 shadow-sm flex-1 min-w-[200px] relative overflow-hidden group hover:border-[#af7a2b] hover:shadow-md transition-all duration-300 min-h-[120px] flex flex-col justify-between animate-fadeIn cursor-pointer">
        ...
        <div className="relative z-10 mt-2 flex items-end justify-between">
            <div className="text-[28px] font-black leading-none text-[#022d41] font-mono" style={{color: colorValue}}>
                {value}
            </div>
            <span className="text-[11px] font-bold uppercase tracking-widest text-[#a3c2d2] flex items-center gap-1">
                {desc}
            </span>
        </div>
    </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#214573] text-[12px] leading-relaxed custom-scrollbar bg-[#e7dedd]/50">
          <section className="animate-fadeIn">
            ...
            <div className="space-y-3 text-[12px] font-medium leading-relaxed bg-white p-4 rounded-xl border border-[#daecf3] shadow-sm">
              ...
              <ul className="list-disc pl-5 space-y-1 text-[#214573]">
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#022d41] mb-3 uppercase flex items-center gap-2 border-b border-[#daecf3] pb-2">
              <Filter size={18} className="text-[#1aa6b7]"/> 2. Status Indicators
            </h4>
            <div className="space-y-3 text-[12px] leading-relaxed font-medium bg-white p-4 rounded-xl border border-[#daecf3] shadow-sm">
              <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded-md bg-[#398797]/10 text-[#398797] border border-[#398797]/20 font-black text-[10px]">SUCCESS</span>
                  <p className="text-[#214573]">การทำงานเสร็จสมบูรณ์ ไม่มีข้อผิดพลาด</p>
              </div>
              <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded-md bg-[#ffa64a]/10 text-[#ffa64a] border border-[#ffa64a]/20 font-black text-[10px]">WARNING</span>
                  <p className="text-[#214573]">การเข้าถึงที่ควรจับตามอง เช่น สิทธิ์ไม่เพียงพอ</p>
              </div>
              <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded-md bg-[#fe424d]/10 text-[#fe424d] border border-[#fe424d]/20 font-black text-[10px]">FAILED</span>
                  <p className="text-[#214573]">การทำงานล้มเหลว หรือการพยายามเจาะระบบ</p>
              </div>
            </div>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[14px] font-black text-[#022d41] mb-3 uppercase flex items-center gap-2 border-b border-[#daecf3] pb-2">
              <Download size={18} className="text-[#398797]"/> 3. Data Export
            </h4>
            <p className="text-[12px] leading-relaxed font-medium">
              ผู้ดูแลระบบสามารถกดปุ่ม <b className="text-[#022d41]">EXPORT LOGS</b> เพื่อดาวน์โหลดข้อมูลเป็นไฟล์ CSV สำหรับนำไปวิเคราะห์ต่อในระบบภายนอก หรือเก็บเป็นหลักฐานรายงาน (Audit Report)
            </p>
          </section>
        </div>
        
        <div className="px-8 py-4 bg-[#f8f9fa] border-t border-[#daecf3] flex justify-end shrink-0">
          <button onClick={onClose} className="px-8 py-2.5 bg-[#022d41] text-white font-black rounded-xl uppercase text-[11px] hover:bg-[#214573] hover:text-white transition-all shadow-md tracking-widest border border-[#022d41]">รับทราบ (Got it)</button>
        </div>
      </div>
    </>,
    document.body
...
function LogDetailsModal({ isOpen, onClose, log }: any) {
    ...
    const getStatusStyle = (status: string) => {
        switch(status) {
            case 'Success': return 'text-[#398797] bg-[#398797]/10 border-[#398797]/20';
            case 'Failed': return 'text-[#fe424d] bg-[#fe424d]/10 border-[#fe424d]/20';
            case 'Warning': return 'text-[#ffa64a] bg-[#ffa64a]/10 border-[#ffa64a]/20';
            default: return 'text-[#a3c2d2] bg-[#f8f9fa] border-[#daecf3]';
        }
    };

    return createPortal(
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-[#022d41]/80 backdrop-blur-md animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-3xl rounded-[28px] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden border border-[#daecf3]/50">
                <div className="bg-[#022d41] px-8 py-4 flex justify-between items-center text-[#af7a2b] shrink-0 border-b border-[#214573]">
                    <div className="flex items-center gap-4">
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-all text-white/70 hover:text-[#fe424d]"><X size={24} /></button>
                </div>

                <div className="p-8 flex flex-col gap-6 bg-[#f8f9fa] overflow-y-auto custom-scrollbar"> 
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="text-2xl font-black text-[#022d41] font-mono tracking-tighter">{log.id}</h2>
                            <p className="text-[11px] font-bold text-[#a3c2d2] mt-1 uppercase tracking-widest">{log.timestamp}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white p-5 rounded-2xl border border-[#daecf3] shadow-sm space-y-4">
                            <h4 className="text-[11px] font-black text-[#022d41] uppercase tracking-widest border-b border-[#e7dedd] pb-2 flex items-center gap-2"><UserCheck size={14} className="text-[#1aa6b7]"/> User Identity</h4>
                            <div className="space-y-3">
                                <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">Username / ID</p><p className="text-[13px] font-black text-[#022d41] font-mono">{log.user}</p></div>
                                <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">Access Role</p><p className="text-[12px] font-bold text-[#af7a2b]">{log.role}</p></div>
                            </div>
                        </div>
                        <div className="bg-white p-5 rounded-2xl border border-[#daecf3] shadow-sm space-y-4">
                            <h4 className="text-[11px] font-black text-[#022d41] uppercase tracking-widest border-b border-[#e7dedd] pb-2 flex items-center gap-2"><Server size={14} className="text-[#1aa6b7]"/> Network & Device</h4>
                            <div className="space-y-3">
                                <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">IP Address</p><p className="text-[13px] font-black text-[#5f7ab7] font-mono">{log.ip}</p></div>
                                <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">User Agent</p><p className="text-[11px] font-medium text-[#214573] truncate" title={log.userAgent}>{log.userAgent}</p></div>
                            </div>
                        </div>
                    </div>

                    <div className="bg-white p-5 rounded-2xl border border-[#daecf3] shadow-sm space-y-4">
                        <h4 className="text-[11px] font-black text-[#022d41] uppercase tracking-widest border-b border-[#e7dedd] pb-2 flex items-center gap-2"><Database size={14} className="text-[#1aa6b7]"/> Action Payload</h4>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">Target Module</p><p className="text-[12px] font-bold text-[#022d41]">{log.module}</p></div>
                            <div><p className="text-[9px] font-black text-[#a3c2d2] uppercase tracking-widest">Action Performed</p><p className="text-[12px] font-black text-[#1aa6b7] uppercase font-mono">{log.action}</p></div>
                        </div>
                        <div className="bg-[#e7dedd] p-4 rounded-xl border border-[#daecf3] font-mono text-[11px] text-[#214573]">
                            <span className="text-[#fe424d] font-bold">"Message"</span>: "{log.details}"
                        </div>
                    </div>
                </div>

                <div className="px-8 py-4 bg-white border-t border-[#daecf3] flex justify-end items-center shrink-0">
                    <button onClick={onClose} className="px-10 py-3 bg-[#022d41] text-white rounded-xl font-black text-[11px] uppercase tracking-widest shadow-md hover:bg-[#214573] hover:text-white transition-all border border-[#022d41]">
                        Close Details
                    </button>
                </div>
            </div>
            ...
            <div className="px-8 pt-3 pb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 z-20 shrink-0">
                <div className="flex items-center gap-5">
                  ...
                  <div className="relative z-10 p-1.5 border border-[#CC0000]/40 rounded-xl bg-white/50 backdrop-blur-sm shadow-sm">
                      <ShieldAlert size={28} strokeWidth={2.5} className="text-[#CC0000]" />
                  </div>
                    </div>
                    <div className="flex flex-col">
                        <div className="flex items-center gap-3">
                            <h3 className="font-black text-[#022d41] uppercase tracking-widest text-[24px] flex items-center gap-2 leading-none">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-[#daecf3]/60 overflow-hidden flex flex-col animate-fadeIn">
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                <div className="relative">
                                    <button onClick={() => setFilterDropdownOpen(!filterDropdownOpen)} className="flex items-center gap-3 bg-[#f8f9fa] px-4 py-2.5 rounded-xl border border-[#daecf3] shadow-sm hover:border-[#af7a2b] hover:bg-white transition-all min-w-[180px]">
                                        ...
                                        <span className="text-[11px] font-black uppercase text-[#022d41]">{statusFilter === 'All' ? 'All Statuses' : statusFilter}</span>
                                        <ChevronDown size={14} className={`text-[#a3c2d2] ml-auto transition-transform ${filterDropdownOpen ? 'rotate-180' : ''}`} />
                                    </button>
                                    {filterDropdownOpen && (
                                        <>
                                            <div className="fixed inset-0 z-40" onClick={() => setFilterDropdownOpen(false)}></div>
                                            <div className="absolute top-[110%] left-0 w-full bg-white border border-[#daecf3] shadow-2xl rounded-2xl p-2 z-50 animate-in fade-in slide-in-from-top-2">
                                                {['All', 'Success', 'Failed', 'Warning'].map(status => (
                                                    <button key={status} onClick={() => { setStatusFilter(status); setFilterDropdownOpen(false); }} className={`w-full flex items-center p-3 rounded-xl transition-all ${statusFilter === status ? 'bg-[#f8f9fa] text-[#022d41]' : 'hover:bg-[#f8f9fa] text-[#a3c2d2]'}`}>
                                ...
                                <div className="relative flex-1 md:w-80">
                                    ...
                                    <input type="text" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Search user, IP, or module..." className="w-full pl-12 pr-4 py-2.5 text-[12px] font-bold rounded-2xl border border-[#daecf3] focus:outline-none focus:border-[#af7a2b] bg-[#f8f9fa] focus:bg-white shadow-sm text-[#022d41] transition-all" />
                                </div>
                            </div>
                            <div className="flex gap-3 shrink-0 w-full md:w-auto">
                                <button className="w-full md:w-auto px-6 py-2.5 bg-gradient-to-r from-[#022d41] to-[#214573] text-white rounded-2xl text-[11px] font-black tracking-widest uppercase shadow-md hover:scale-105 transition-all flex items-center justify-center gap-2 border border-[#022d41]">
                        ...
                        <div className="overflow-x-auto custom-scrollbar bg-[#f8f9fa]">
                            <table className="w-full text-left font-sans border-collapse">
                                ...
                                <tbody className="divide-y divide-[#daecf3] bg-white">
                                    {paginatedLogs.length > 0 ? paginatedLogs.map((log) => (
                                        <tr key={log.id} className="hover:bg-[#f8f9fa] transition-colors group">
                                            <td className="py-3 px-6 text-[12px]">
                                                <div className="flex items-center gap-2 font-mono text-[#a3c2d2]">
                                                    <Clock size={14}/> <span>{log.timestamp}</span>
                                                </div>
                                            </td>
                                            <td className="py-3 px-6">
                                                <div className="flex flex-col">
                                                    <span className={`font-black text-[12px] font-mono tracking-tight ${log.user === 'UNKNOWN_USER' ? 'text-[#fe424d]' : 'text-[#022d41]'}`}>{log.user}</span>
                                                    <span className="text-[10px] font-bold text-[#a3c2d2] mt-0.5 uppercase tracking-wider">{log.role}</span>
                                                </div>
                                            </td>
                                            <td className="py-3 px-6 font-mono text-[12px] font-bold text-[#1aa6b7]">
                                                <div className="flex items-center gap-1.5"><MapPin size={12} className="text-[#a3c2d2]"/> {log.ip}</div>
                                            </td>
                                            <td className="py-3 px-6">
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-[12px] text-[#022d41]">{log.module}</span>
                                            ...
                                            <td className="py-3 px-6 text-center">
                                                <div className="flex justify-center items-center gap-[1px]">
                                                    <button onClick={() => setSelectedLog(log)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-[#daecf3] text-[#1aa6b7] hover:bg-[#e7dedd] hover:border-[#1aa6b7] transition-all active:scale-90" title="View Details">
                                                        <Eye size={14} />
                                                    </button>
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            <td colSpan="6" className="py-12 text-center text-[#a3c2d2] font-bold text-[12px]">
                        ...
                        <div className="px-8 py-3 bg-[#f8f9fa] border-t-[1.5px] border-slate-300 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
                            <div className="flex items-center gap-6 text-[11px] font-black text-[#a3c2d2] uppercase tracking-widest">
                                <div className="flex items-center gap-3">
                                    ...
                                    <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="bg-white border border-[#daecf3] rounded-lg px-3 py-1.5 outline-none font-black text-[#022d41] cursor-pointer shadow-sm focus:border-[#af7a2b]">
                                        {[15, 30, 50, 100].map(v => <option key={v} value={v}>{v}</option>)}
                                    </select>
                                </div>
                                <p className="bg-white px-4 py-1.5 rounded-lg border border-[#daecf3] shadow-sm text-[#022d41]">Total Records: {filteredLogs.length}</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className={`w-9 h-9 border border-[#daecf3] bg-white rounded-lg flex items-center justify-center transition-all ${currentPage === 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#e7dedd] text-[#1aa6b7] shadow-sm active:scale-90'}`}>
                                    <ChevronLeft size={16}/>
                                </button>
                                <div className="bg-white text-[#022d41] px-6 py-2 rounded-lg font-black text-[11px] min-w-[120px] text-center uppercase tracking-widest border border-[#daecf3] shadow-sm">
                                    Page {currentPage} / {totalPages || 1}
                                </div>
                                <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className={`w-9 h-9 border border-[#daecf3] bg-white rounded-lg flex items-center justify-center transition-all ${currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#e7dedd] text-[#1aa6b7] shadow-sm active:scale-90'}`}>
